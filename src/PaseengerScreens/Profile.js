...
const Profile = ({ navigation }) => {
    ...
    const fetchUserData = async () => {
      try {
        ...
        if (currentUser) {
          ...
          const response = await axios.get(`http://192.168.100.4:5000/rider?email=${currentUser.email}`);
  ...
  const handleUpdate = async () => {
    try {
      ...
      await axios.put('http://192.168.100.4:5000/update-profile', {
        email: user.email,
        username: user.name
      });
        ...
        <View style={styles.profileSection}>
          ...
          >
            {uploading ? (
              ...
              <Image
                source={{ uri: `http://192.168.100.4:5000${user.photoUrl}` }}
